                ...
                if response_manager_url.status == 200:
                    ...
                    if response_restart.status in [200, 204]:  
                        ...
                        try:
                            ...
                            error_message = response_restart
                        result = {'ret': False, 'msg': "response restart Error code %s \nerror_message: %s" % (response_restart.status, error_message)}
                        return result
                else:
                    try:
                    ...
                    result = {'ret': False, 'msg': "response manager url Error code %s \nerror_message: %s" % (response_manager_url.status, error_message)}
                    return result
        else:
            try:
