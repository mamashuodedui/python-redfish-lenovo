        ...
        for i in range(len(system)):
            ...
            if response_system_url.status == 200:
                ...
                try:
                    ...
                    error_message = response_system_url
                result = {'ret': False, 'msg': "response system url Error code %s \nerror_message: %s" % (response_system_url.status, error_message)}
                return result
            ...
            if response_bios_url.status == 200:
                ...
                if response_reset_bios.status == 200:
                    ...
                    try:
                        ...
                        error_message = response_reset_bios
                    result = {'ret': False, 'msg': 'response reset bios Error code %s \nerror_message: %s'% (response_reset_bios.status, error_message)}
                    return result
            else:
                try:
                    error_message = utils.get_extended_error(response_bios_url)
                except:
                    error_message = response_bios_url
                result = {'ret': False, 'msg': "response bios url Error code %s \nerror_message: %s" % (response_bios_url.status, error_message)}
                return result
    except Except as e:
