        ...
        for request in response_manager_url.dict['Members']:
            ...
            if response_url.status == 200:
                ...
                if response_config_url.status == 200:
                    ...
                    if response_reset_default_url.status == 200:
                        result = {'ret': True,
                                  'msg': "reset bmc configuration default succesfully"}
                        REDFISH_OBJ.logout()
                        return result
                    else:
                        error_message = utils.get_extended_error(response_reset_default_url)
                        result = {'ret': False, 'msg': "Url '%s' response Error code %s \nerror_message: %s" % (
                            response_reset_default_url, response_reset_default_url.status, error_message)}
                        REDFISH_OBJ.logout()
                        return result
                else:
