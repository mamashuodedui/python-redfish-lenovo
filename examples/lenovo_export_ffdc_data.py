        ...
        for i in managers_list:
            ...
            if response_manager_uri.status == 200:
                ...
                try:
                    ...
                    error_message = response_manager_uri
                result = {'ret': False, 'msg': "response manaegr uri Error code %s \nerror_message: %s" % (response_manager_uri.status, error_message)}
                return result
                    ...
                    while True:
                        ...
                        if response_task_uri.status == 200:
                            ...
                            try:
                                ...
                                error_message = response_task_uri
                            result = {'ret': False, 'msg': "response task uri Error code %s \nerror_message: %s" % (response_task_uri.status, error_message)}
                            break

                    # Delete the task when the taskstate is completed
                    response_delete_task = REDFISH_OBJ.delete(task_uri, None)
                else:
                    try:
                        error_message = utils.get_extended_error(response_ffdc_data_uri)
                    except:
                        error_message = response_ffdc_data_uri
                    result = {'ret': False, 'msg': "response manaegr uri Error code %s \nerror_message: %s" % (response_ffdc_data_uri.status, error_message)}
                    return result
            else:
                try:
                    error_message = utils.get_extended_error(response_servicedata_uri)
                except:
                    error_message = response_servicedata_uri
                result = {'ret': False, 'msg': "response manaegr uri Error code %s \nerror_message: %s" % (response_servicedata_uri.status, error_message)}
                return result
